...
Console.WriteLine("\n"
    ...
    + "Напишите программу, которая внутри данной матрицы, заменяет строки на столбцы.\n"
...
double[,] RotateMatrix(double[,] array)
{
    int length = array.GetLength(1);
    int height = array.GetLength(0);
    if(length!=height) return array;
...
bool CanRotateMatrix(double[,] array){
    if(array.GetLength(0) != array.GetLength(1)) return false;
    return true;
}
void main()
{
    ...
    System.Console.WriteLine();
    Console.ResetColor();
    if(!CanRotateMatrix(matrix))
    {
        PrintString("Для Выполнения задачи","матрица должна быть квадратная!");
        return;
    } 
